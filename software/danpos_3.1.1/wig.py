...
class Wig:
    ...
    def fillRegions(self,regions={},file=None,pheight=1,height=0,width=0,calculate_P_value=1,pos_only=False,suppress=False):
        ...
        for chrom in regions:
            ...
            for start in starts:
                ...
                lst = dic[chrom] if chrom in dic else numpy.array([0.0])
                if lst.size<=e:lst.resize(e+1,refcheck=0)
                width_above_cutoff=0
                v=max(lst[s:e])
                ...
                while i<e:
                    '''
                    if substart<0:
                        if lst[i]>=height:substart=i
                    elif lst[i]<height:
                        subpeaks+=str(substart*step)+':'+str(i*step)+','
                        substart=-1
                    '''
                    if lst[i]>=height:width_above_cutoff+=step
                    if pos_only:
                        if lst[i]>0:auc+=lst[i]
                    else:auc+=lst[i]
                    i+=1
    ...
    def callSummits(self,width=40,pcut=1,height=5,regions=None):
        ...
        for cr in regions:#self.data:
            if not cr in self.data: continue
